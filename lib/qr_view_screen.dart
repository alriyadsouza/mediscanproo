...
import 'package:mediscanproo/MedicationReminderPage.dart';
...
class _QRViewScreenState extends State<QRViewScreen> {
  ...
  void notifyMe() {
    // Extract the medicine name using a regular expression
    String medicineName = extractMedicineName(result?.code ?? "");

    if (medicineName.isNotEmpty) {
      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => MedicationReminderPage(initialMedicineName: medicineName),
        ),
      );
    } else {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Medicine name not found in scanned data.'),
        ),
      );
    }
  }

  String extractMedicineName(String scannedData) {
    final RegExp regExp = RegExp(r'Medicine Name:\s*(.+)');
                  ...
                  children: [
                    if (result != null) ...[
                      ...
                      ElevatedButton(
                        ...
                        onPressed: () {
                          notifyMe(); // Call the function directly without wrapping it inside a MaterialPageRoute
                        },
